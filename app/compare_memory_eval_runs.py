...
def load_memory_eval_runs() -> list[dict[str, Any]]:
    if not MEMORY_EVAL_RESULTS_FILE.exists():
        return []

    records: list[dict[str, Any]] = []

    with MEMORY_EVAL_RESULTS_FILE.open("r", encoding="utf-8") as f:
...
def compare_two_runs(
    ...
    print("\n" + "=" * 60)
    print("MEMORY EVAL RUN COMPARISON")
    print("=" * 60)
    ...
    if interesting_case_ids:
        ...
        for case_id in interesting_case_ids:
            ...
            if prev_result:
                ...
                print(f"  이전 operation: {prev_details.get('operation_type')}")
                print(f"  이전 missing: {prev_details.get('missing')}")
                print(f"  이전 found_forbidden: {prev_details.get('found_forbidden')}")

            if latest_result:
                latest_details = latest_result.get("details", {})
                print(f"- 최신 passed: {latest_result.get('passed')}")
                print(f"  최신 operation: {latest_details.get('operation_type')}")
                print(f"  최신 missing: {latest_details.get('missing')}")
                print(f"  최신 found_forbidden: {latest_details.get('found_forbidden')}")


def main() -> None:
    runs = load_memory_eval_runs()

    if not runs:
        print("비교할 memory eval 결과가 없습니다. 먼저 memory_eval_runner를 실행하세요.")
        return

    if len(runs) < 2:
        print("비교하려면 최소 2개의 memory eval 실행 결과가 필요합니다.")
