...
### 3. batch approval first-turn 정책 검증 / 튜닝
- 왜 중요한지: 구체적인 복수 파일 변경 요청과 빈 폴더 생성 요청에서 첫 턴에 `request_batch_operations` approval pending 을 만들도록 prompt/schema/hint 가 강화됐지만, 실제 모델별 tool-calling 준수율은 추가 확인이 필요하다.
- 선행조건 / 의존성: 현재 `build_system_prompt()` batch approval 정책, `should_hint_immediate_batch_approval()`, planner batch hint, `request_batch_operations` schema description, `create_directory` batch operation 유지
- 기대 결과물: 구체적 batch 요청, 단일 빈 폴더 생성 요청, 폴더+파일 혼합 요청, 모호한 batch 요청, 읽기/분석/요약 의존 요청을 구분하는 trace/eval 기준과 필요 시 hint rule 조정안
- 검증 방법: `uv run pytest -q tests/test_agent.py`, `uv run pytest -q tests/test_workspace_tools.py`, `uv run pytest -q`, 실제 Web/CLI 에서 구체적 batch 요청과 빈 폴더 생성 요청이 첫 턴 pending approval 로 이어지는지 수동 확인
- 실행 전에 사람 승인이 필요한지 여부: 예
...
### 5. Web UI 상태 orchestration 남은 경계 점검
...
### 6. planner/execution summary 와 workspace state 연결 검토
...
## Next
### 7. model/provider routing 정책 초안 구체화
...
### 9. tool panel / observability 패널 재설계
...
### 10. memory suggestion optional mode / eval 경계 정리
