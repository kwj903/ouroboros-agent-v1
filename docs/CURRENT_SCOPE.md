...
- Web UI 운영 경험을 어떻게 더 안정적으로 다듬을지
...
- `frontend/src/App.tsx` 는 더 이상 단일 렌더링 파일만은 아니지만, session/data orchestration 로직은 여전히 많이 남아 있다.
- `frontend/src/components/OperationsSidebar.tsx` 가 크고 책임이 넓어 approval/memory/observability UI 가 더 나뉠 여지가 있다.
...
- `plan_tasks()` 를 항상 호출하는 구조를 유지할지, 조건부 planning 으로 바꿀지
